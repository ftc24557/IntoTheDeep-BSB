...
@Autonomous(name = "5+0off", group = "BSB")
public class Auto5plus0off extends OpMode {
    ...
    Pose startingPose = new Pose(10,45,0);
    ...
    Pose scoringPose5 = new Pose(36.95, 72, 0);
    Pose scoringPose6 = new Pose(36,95, 68);
    private Path scorePreload, park;
    private PathChain   push3ToPickUp1, score2, score2ToPickUp2, score3, score3ToPickUp3, score4, score4ToPickUp4, score5, score5ToPickUp5, score6;
    private Path preloadToPush, startingToPush, push1, push1ToPush2, push2, push2ToPush3, push3;
    ...
    public void buildPaths(){
        ...
        startingToPush = new Path(new BezierCurve(new Point(startingPose),
                new Point(new Pose(14,31)),
                new Point(startPush1)));
        startingToPush.setLinearHeadingInterpolation(startingPose.getHeading(), startPush1.getHeading());
        ...
        score4 = follower.pathBuilder().addPath(new BezierCurve(
                ...
                .build();
        score4ToPickUp4 = follower.pathBuilder().addPath(new BezierCurve(
                        new Point(scoringPose4),
                        new Point(16,27),
                        new Point(pickUpPose)
                ))
                ...
                .setPathEndVelocityConstraint(0.5)
                .setLinearHeadingInterpolation(scoringPose3.getHeading(), pickUpPose.getHeading()).build();
        score5 = follower.pathBuilder().addPath(new BezierCurve(
                        new Point(pickUpPose),
                        new Point(20, 50),
                        new Point(scoringPose5)
                )).setLinearHeadingInterpolation(0,0)
                .build();
        score5ToPickUp5 = follower.pathBuilder().addPath(new BezierCurve(
                        new Point(scoringPose5),
                        new Point(16,27),
                        new Point(pickUpPose)
                ))
                ...
                .setPathEndVelocityConstraint(0.5)
                .setLinearHeadingInterpolation(scoringPose3.getHeading(), pickUpPose.getHeading()).build();
        score6 = follower.pathBuilder().addPath(new BezierCurve(
                        new Point(pickUpPose),
                        new Point(20, 50),
                        new Point(scoringPose6)
                )).setLinearHeadingInterpolation(0,0)
                .build();

        park = new Path(new BezierLine(new Point(scoringPose6),new Point(pickUpPose)));
    ...
    public void autonomousPathUpdate() {
        switch (pathState) {
            ...
            case 11:
                if (!follower.isBusy()){
                    outtake.SetState(OuttakePositional.state.INTAKE_WALL);
                    follower.followPath(score4ToPickUp4,true);
                    setPathState(12);
                }
                break;
            case 12:
                if (!follower.isBusy()){
                    if (!inDelay){
                        inDelay = true;
                        Alarm score5Alarm = new Alarm(300, ()->{
                            inDelay = false;
                        });
                        score5Alarm.Run();
                    }
            ...
            case 13:
                if (!follower.isBusy()){
                    outtake.SetState(OuttakePositional.state.INTAKE_WALL);
                    follower.followPath(score5ToPickUp5,true);
                    setPathState(14);
                }
                break;
            case 14:
                if (!follower.isBusy()){
                    if (!inDelay){
                        inDelay = true;
                        Alarm score6Alarm = new Alarm(300, ()->{
                            inDelay = false;
                        });
                        score6Alarm.Run();
                    }
